...
| `web_search` | `boolean` | `false` | When `true`, the gateway searches the live web, screens every page through the GLAD-BERT firewall, and grounds the answer in the safe pages (citing sources). Requires `GW_WEBSEARCH_ENABLED=1` (default). With a Tavily key configured it uses Tavily, otherwise the key-less DuckDuckGo fallback. See [Live Web Search](web-search.md). |
