...
## Remote G1-Hummingbird scoring pool

By default the G1-Hummingbird detector runs **embedded** inside the gateway as an in-process singleton monitor. For horizontal scale you can offload detection to a separate GPU pool so that engine replicas can be CPU-only and the GPU scales independently.
