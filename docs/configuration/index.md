...
```yaml
watermark:
  verify_url: "https://verify.geodesia.ai/watermark"
```

| Field | Default | Description |
|---|---|---|
| `verify_url` | `"https://verify.geodesia.ai/watermark"` | Verification URL reported by the product backend's compliance engine (`latent_watermark.verifiable_at`). |

The latent watermark itself (HMAC-SHA256, method `hmac_sha256_v1`) is always computed and logged for every
answer the gateway returns; it is **not** added to the API response, and there is no setting that adds it.
Older versions of this page listed `enabled`, `algorithm`, `include_in_response`, `disclosure_text` and
`custom_disclosure`: the current build does not read them. See [AI Watermark](../compliance/watermark.md).
