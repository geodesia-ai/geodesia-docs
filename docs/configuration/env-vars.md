...
| `GW_ADDITIONAL_AXES` | `profanity,out_of_scope,prompt_complexity` | Which axes are **additional**: they travel in the response's `geodesia.additional_axes` object instead of `geodesia.axes`, and cannot be promoted to blocking by configuration (the signed certificate marks them `tier: "additional"`). Set it to `""` to make every axis primary. See [Response Format](../reference/response-format.md#axes). |
