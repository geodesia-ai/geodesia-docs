...
On **older Ollama (< 0.12)** that predates native log-probabilities, you can still recover closed-book detection with a **logprob sidecar**: a second instance of `llama.cpp` or another OpenAI-compatible server serving the same model with log-probability support. The gateway deterministically re-derives the answer through the sidecar to recover the needed signals. On Ollama ≥ 0.12 this is unnecessary.
