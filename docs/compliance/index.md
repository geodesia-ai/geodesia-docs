...
Inference call
    │
    ├── Pre-flight: Kill-switch check → block if active
    │
    ├── Generate + score → detection results
    │
    ├── Post-inference:
    │   ├── Watermark injection (latent HMAC-SHA256)
    │   ├── Append-only log entry (hash-chained)
    │   ├── Retention tagging (expiry date assigned)
    │   └── Oversight trigger (if scores exceed oversight thresholds)
    │
    └── Response delivered
```
