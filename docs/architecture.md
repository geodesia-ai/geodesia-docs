...
| **Opt-in** | **GLAD-Tapestry** | A heavyweight, proprietary 8B guardian symbiont loaded in-process in 4-bit. Reads the *full geometry* of the exchange and returns a confident second opinion that is blended into the safety and hallucination axes. Off by default — never loaded, zero overhead. | When **Deep Scan** is enabled (see [Deep Scan](gateway/deep-scan.md)) |
